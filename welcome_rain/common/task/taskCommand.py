...
class GmondInstallExecute(BaseCommand):
    ...
    def run(self,task,url="",port=80,task_data=""):
        ...
        command = 'apt-get -y install ganglia-monitor'
...
class GmetadInstallExecute(BaseCommand):
    def checkTask(self):        
    ...
    def run(self,task,url="",port=80,task_data=""):
        ...
        command = 'apt-get -y install gmetad'
        result = self.commandSudo(command)
        self.setResult(task)
        self.postResult(task, url, port)
        self.postTaskStatus(task, url, port)
...
class ServerInspectionExecute(BaseCommand):
    def parseUname(self,tokens):
        os = {}
        os['kernel'] = tokens[0]
        os['node_name'] = tokens[1]
        os['release'] = tokens[2]
        os['version']= tokenss[3]
        os['machine'] = tokens[4]
        os['processor'] = tokens[5]
        os['hardware'] = tokens[6]
        os['system'] = tokens[7]

        return os
        

    def setLinuxTypeResult(self,task):
        ...
        os = self.parseUname(output.output.split(" "))
        task.setStatus(taskStatus)
    
    def setLinuxRelaseResult(self,task):
    ...
    def postResult(self,task,url,port):
        ...
        poster = HTTPTaskPoster(url,port,action=API_UPDATE_SERVER)
        poster.addParameter("server_id", task.serverID)
        poster.addParameter("server_version", task.taskOutput)
        poster.addParameter("gmond_install_flag", "")
        poster.addParameter("gmetad_install_flag", "")
        poster.addParameter("plugin_list", "")
        poster.addParameter("description", "")
    ...
    def getLinuxType(self,task):
        env = self.getFabricEnvironment()
        self.commandSudo('uname -a')
        self.setResult(task)
        
    def getReleaseInfo(self,task):
        env = self.getFabricEnvironment()
        self.commandSudo('cat /etc/*-release')
        self.setResult(task)
...
class GmondPluginInstallExecute(BaseCommand):
    def checkTask(self):        
