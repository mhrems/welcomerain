...
Flotr.addPlugin('graphGrid', {
  ...
  drawGrid: function(){
    ...
    //var point = Math.floor(this.axes.x.d2p(event[0]))
    //console.log(point)
    ...
    if (events){
    	...
    	for (var index =0; index< events.length ; index++){
    		...
    		//console.log(event);
    		//console.log(Math.floor(this.axes.x.d2p(2.0)) + ctx.lineWidth/2 + this.plotOffset.left);
    		var point = Math.floor(this.axes.x.d2p(event[0])) + this.plotOffset.left
    		//console.log(point);
    		ctx.moveTo(point, 0);
    		ctx.lineTo(point, plotHeight);
    		ctx.strokeStyle = event[1];
    		//ctx.strokeStyle = 'red';
    		ctx.stroke();
    	}
    }
